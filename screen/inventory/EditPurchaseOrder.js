...
export default class EditPurchaseOrder extends React.Component {
      ...
      () => {
        ...
        if (typeof selectedItemID === "undefined") {
          ...
          let index = addedItems.findIndex(
            (element) =>
              element.product_code === selectedItemID &&
              element.store_id === storeNameID
          );
  ...
  renderItem = (item, index) => (
    <View
      key={
        item.product_code + "-" + item.store_id + "-" + index + "-" + item.id
      }
    ...
    >
      ...
      <View style={styles.itemRowBody}>
        ...
        >
          <MenuItem onPress={this.editItem.bind(this, item.product_code)}>
            Edit
          </MenuItem>
          <MenuItem
