      ...
      if (status.granted) {
        ...
        ImagePicker.launchImageLibraryAsync(optins).then(async (result) => {
          if (!result.cancelled) {
            ...
            // let medres = await MediaLibrary.getAssetInfoAsync(asset);
            //  let medres = await MediaLibrary.getAssetInfoAsync({
            //   id: `${Number(this.state.imageID) + 1}`,
            //   uri: await MediaLibrary.createAssetAsync(result.uri),
            // });
            // console.log({ medres: result.exif });
            this.setState({
              Images: [
                ...this.state.Images,
