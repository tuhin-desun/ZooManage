...
export const todoList = (user_id, query, adv) => {
  if (query && !adv) {
    ...
    return ssoInstance.get(
      Config.TASK_URL + "category/" + user_id + "?filter=" + query
    );
  } else if (!query && adv) {
    ...
    console.log(
      Config.TASK_URL + "category/" + user_id + "?filter=" + query + "&" + adv
    );
    return ssoInstance.get(
      Config.TASK_URL + "category/" + user_id + "?filter=" + query + "&" + adv
    );
...
export const subcat = (cat_id, user_id, query, adv) => {
  if (query && !adv) {
    ...
    console.log(
      Config.TASK_URL + "category/" + cat_id + "/" + user_id + "?" + adv
    );
    return ssoInstance.get(
      Config.TASK_URL + "category/" + cat_id + "/" + user_id + "?" + adv
    );
    ...
    return ssoInstance.get(
      Config.TASK_URL + "category/" + cat_id + "/" + user_id
    );
  }
};
...
export const subCategoryList = (category_id, user_id, query, adv, page) => {
  if (query && !adv) {
    ...
    console.log(
      Config.TASK_URL + "task/" + category_id + "/" + user_id + "?page=" + page
    );
    return ssoInstance.get(
      Config.TASK_URL + "task/" + category_id + "/" + user_id + "?page=" + page
    );
...
export const getPendingTasks = (category_id, user_id) => {
  ...
  return ssoInstance.get(
    Config.TASK_URL + "pending_task/" + category_id + "/" + user_id
  );
};
...
export const usersTaskListWithSubcat = (user_id, subcat_id) => {
  return ssoInstance.get(
    Config.TASK_URL + "task/get_user_task/" + user_id + "/" + subcat_id
  );
};
